...
signed main() {
    ...
    std::vector<std::string> alphabet_postfixes = {"binary", "quaternari"};
    ...
    std::vector<std::string> benchmarks = {generateRandomString(2, 10000), generateRandomString(2, 100000), generateRandomString(4, 10000),
                                           generateRandomString(4, 100000)};
        ...
        *tables[table] << '\n';
        for (int search = 0; search < static_cast<int>(search_functions.size()); ++search) {
            for (int type = 0; type < 5; ++type) {
                ...
                *tables[table] << search_names[search] << type_postfixes[type];

                for (int size = 100; size <= 3000; size += 100) {
                    int position = rand() % ((table % 2 ? 100000 : 10000) - size);
                    std::string pattern = benchmarks[table].substr(position, size);
                    ...
                    for (int count = 0; count < static_cast<int>(kMeasurementNumber); ++count) {
                        ...
                        search_functions[search](pattern, benchmarks[table]);
